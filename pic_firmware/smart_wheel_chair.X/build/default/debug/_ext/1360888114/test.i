...
# 19 "../APP/test.c" 2
# 1 "../APP/../MCAL/TMR0/TMR0_interface.h" 1
...
# 20 "../APP/test.c" 2
# 1 "../APP/../MCAL/EXTI/EXTI_interface.h" 1
...
# 23 "../APP/test.c" 2
# 1 "../APP/../HAL/LCD/LCD_interface.h" 1
...
# 25 "../APP/test.c" 2
# 34 "../APP/test.c"
static void LCD_ShowUltrasonic(u16 dist) {
    Lcd_Cmd(0x80);
    if (dist >= 400) {
        Lcd_PrintString("Dist: >400 cm   ");
    } else {
        Lcd_PrintString("Dist: ");
        Lcd_Out_Int((s16)dist);
        Lcd_PrintString(" cm    ");
    }

    Lcd_Cmd(0xC0);
    if (dist <= 25) Lcd_PrintString("!! STOP !!      ");
    else if (dist <= 80) Lcd_PrintString("WARNING         ");
    else if (dist < 400) Lcd_PrintString("Safe            ");
    else Lcd_PrintString("Out of range    ");
}


static void UART_PrintUltrasonic(u16 dist) {
    UART_SendString("\r\n[ULTRA] Dist: ");
    UART_SendNum(dist);
    UART_SendString(" cm");
}
...
void main(void) {
    ...
    Lcd_Cmd(0x80); Lcd_PrintString("System Ready    ");
    UART_SendString("\r\n[BOOT] Ultrasonic Test Started\r\n");
    ...
    while (1) {
        ...
        if (distance <= 25) { PORTBbits.RB1 = 1; PORTBbits.RB2 = 1; }
        else if (distance <= 80) { PORTBbits.RB1 = 1; PORTBbits.RB2 = 0; }
        else { PORTBbits.RB1 = 0; PORTBbits.RB2 = 0; }


        LCD_ShowUltrasonic(distance);


        UART_PrintUltrasonic(distance);


        _delay((unsigned long)((200u)*(10000000UL/4000.0)));
    }
}
