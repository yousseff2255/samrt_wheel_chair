# 1 "../HAL/MAX30102/MAX30102_program.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 295 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "../HAL/MAX30102/MAX30102_program.c" 2
# 11 "../HAL/MAX30102/MAX30102_program.c"
...
# 12 "../HAL/MAX30102/MAX30102_program.c" 2

# 1 "../HAL/MAX30102/../../SERVICES/STD_TYPES.h" 1
...
# 14 "../HAL/MAX30102/MAX30102_program.c" 2
# 1 "../HAL/MAX30102/../../MCAL/I2C/I2C_interface.h" 1
# 11 "../HAL/MAX30102/../../MCAL/I2C/I2C_interface.h"
...
# 15 "../HAL/MAX30102/MAX30102_program.c" 2
# 1 "../HAL/MAX30102/MAX30102_interface.h" 1
...
# 16 "../HAL/MAX30102/MAX30102_program.c" 2
# 1 "../HAL/MAX30102/MAX30102_private.h" 1
# 34 "../HAL/MAX30102/MAX30102_private.h"
typedef struct {
    int32_t prevDC_scaled;
} DC_Filter_t;
...
# 17 "../HAL/MAX30102/MAX30102_program.c" 2
# 1 "../HAL/MAX30102/MAX30102_config.h" 1
# 18 "../HAL/MAX30102/MAX30102_program.c" 2
...
static uint16_t s_bpmBuffer[4u] = {0};
static uint8_t s_bpmIdx = 0;
static uint8_t s_bpmCount = 0;

extern volatile uint32_t g_systemTick_ms;
# 45 "../HAL/MAX30102/MAX30102_program.c"
static int16_t DC_Removal(uint32_t raw, DC_Filter_t *f) {

    f->prevDC_scaled = (19L * f->prevDC_scaled + (int32_t)raw * 20L) / 20L;


    int32_t ac = (int32_t)raw - (f->prevDC_scaled / 1024L);
...
static int16_t MA_Filter(int16_t in, MA_Filter_t *f) {
    f->buf[f->idx] = in;
    f->idx = (uint8_t)((f->idx + 1u) % 4u);
    int32_t sum = 0;
    uint8_t i;
    for (i = 0u; i < 4u; i++) sum += f->buf[i];
    return (int16_t)(sum / (int32_t)4u);
}
...
static void BPM_Push(uint16_t bpm) {
    ...
    s_bpmIdx = (uint8_t)((s_bpmIdx + 1u) % 4u);
    if (s_bpmCount < 4u) s_bpmCount++;
...
MAX30102_Status_t MAX30102_Init(void) {
    MAX30102_WriteReg(0x09u, 0x40u);
    _delay((unsigned long)((100)*(10000000UL/4000.0)));
    MAX30102_WriteReg(0x09u, 0x03u);
    MAX30102_WriteReg(0x08u, 0x0Fu);
    MAX30102_WriteReg(0x0Au, 0x27u);
    MAX30102_WriteReg(0x0Cu, 0x24);
    MAX30102_WriteReg(0x0Du, 0x24);
    return MAX30102_OK;
}
...
void MAX30102_Update(void) {
    ...
    uint8_t wrPtr = MAX30102_ReadReg(0x04u);
    uint8_t rdPtr = MAX30102_ReadReg(0x06u);
    if (wrPtr == rdPtr) return;
    ...
    I2C_Repeated_Start();
    I2C_Write((0x57u << 1) | 1u);
    for (i = 0u; i < 5u; i++) buf[i] = I2C_Read(1);
    buf[5] = I2C_Read(0);
    I2C_Stop();
    ...
    if (ir < 5000UL) {
        ...
        s_bpmIdx = 0;
        s_bpmCount = 0;
        for (i = 0u; i < 4u; i++) s_bpmBuffer[i] = 0u;
        for (i = 0u; i < 4u; i++) irMA.buf[i] = 0;
        return;
    }
